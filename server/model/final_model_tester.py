...
HIDDEN_SIZE   = 256
NUM_CLASSES   = 2  # binary: Safe vs Danger
OBSERVE_RATIO = 0.6
BATCH_SIZE    = 16
SEQUENCE_LEN  = 30
...
# Confidence thresholds for early warning system
# When predicting Danger, the confidence level determines alert severity
ALERT_THRESHOLDS = {
    'elevated': 0.35,  # ⚠️  rising danger signal
    'high':     0.60,  # 🚨 confirmed danger
}
...
def run_dataset(model, data_dir, split='val'):
    ...
    danger_mask = (all_labels == 1)
    if danger_mask.sum() > 0:
        tp  = ((all_preds == 1) & danger_mask).sum().item()
        fp  = ((all_preds == 1) & ~danger_mask).sum().item()
        fn  = ((all_preds == 0) & danger_mask).sum().item()
        pre = tp / (tp + fp + 1e-6)
        rec = tp / (tp + fn + 1e-6)
        f1  = 2 * pre * rec / (pre + rec + 1e-6)
        print(f"\n  Precision : {pre*100:.1f}%")
        print(f"  Recall    : {rec*100:.1f}%  ← most important (missing a fight = bad)")
...
def run_webcam(model, cam_id=0):
    ...
    while True:
        ...
        # ── Determine alert level from confidence ────────────────────────
        danger_prob = last_probs[1] if len(last_probs) > 1 else 0.0
        if smooth_pred == 0:
            alert_level = 'SAFE'
            color       = CLASS_COLORS[0]
        elif danger_prob >= ALERT_THRESHOLDS['high']:
            alert_level = '🚨 DANGER'
            color       = (0, 0, 220)   # red
        elif danger_prob >= ALERT_THRESHOLDS['elevated']:
        ...
        status = f"{alert_level}  {danger_prob*100:.0f}%"
