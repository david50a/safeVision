...
class SecureVideoStreamWithDH:
    ...
    def _hmac(self, key: bytes, data: bytes) -> bytes:
        # hmac_lib expects lists of ints and returns a list of ints
        result = hmac_lib.hmac_sha256(list(key), list(data))
        return bytes(result)
    @staticmethod
    def _recv_exactly(sock:socket.socket,n:int)->bytes:
        ...
        try:
            while len(data)<n:
                ...
                if not packet:
                    raise RuntimeError(f"Socket connection broken - expected {n} bytes, got {len(data)}")
                data+=packet
        except socket.timeout:
            raise TimeoutError(f"Socket timeout while receiving {n} bytes - got {len(data)}")
